...
TEST(LogTest, LogCreator) {
  ...
  lc.set_nblk_seg(12);
  lc.set_nseg_flash(1000);
  ASSERT_TRUE(lc.VerifyMetrics());
  ASSERT_EQ((size_t)(FLASH_SECTOR_SIZE * 4 * 12 * 1000), lc.noctet_flash());

  char vdname[L_tmpnam] = P_tmpdir "/vflashXXXXXX";
  int fd = ::mkstemp(vdname);
  ::ftruncate(fd, lc.noctet_flash());
  ::close(fd);
  VFlash flash;
  flash.Open(vdname);
  ASSERT_TRUE(lc.set_flash(&flash));
  ASSERT_TRUE(lc.WriteSuperblock());

  Superblock sb;
  ASSERT_TRUE(sb.Read(&flash));
  EXPECT_EQ((size_t)FLASH_SECTOR_SIZE, sb.noctet_sector());
  EXPECT_EQ((FlashSectorNum)4, sb.nsector_blk());
  EXPECT_EQ((size_t)(FLASH_SECTOR_SIZE * 4), sb.noctet_blk());
  EXPECT_EQ((BlkNum)12, sb.nblk_seg());
  EXPECT_EQ((SegNum)1000, sb.nseg_flash());
