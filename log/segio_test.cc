...
class LogTestSegIOSuperblock : public Superblock {
  public:
    LogTestSegIOSuperblock(size_t noctet_sector, FlashSectorNum nsector_blk, BlkNum nblk_seg, SegNum nseg_flash) {
...
TEST(LogTest, SegIO) {
  ...
  const FlashSectorNum segment_size = 4;
  ...
  ASSERT_TRUE(segio.Read(seg1, buf2));//seg1 is cached, the change directly on flash is not reflected
  EXPECT_EQ(0, std::memcmp(buf1, buf2, superblock.noctet_seg()));
}
