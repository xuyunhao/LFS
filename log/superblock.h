...
class Superblock {
  public:
    ...
    FlashSectorNum nsector_seg(void) const { return this->nsector_blk() * this->nblk_seg(); }//count of sectors in segment
  ...
  protected:
    ...
    void set_nsector_blk(FlashSectorNum value);
    void set_nblk_seg(BlkNum value);
    void set_first_logseg(SegNum value);
    void set_nseg_flash(SegNum value);
    void set_checkpoint_seg(int index, SegNum value);
    void set_nseg_checkpoint(SegNum value);
    void set_nblk_segsummary(BlkNum value);
    FlashSectorNum FindBlockSize(Flash* flash);//search sectors for kMagic and determine nsector_blk, return 0 if kMagic is not found in first 1025 sectors
