...
SegBlkNum BlkWriter::Put(BlkInfo* bi, const uint8_t* buf) {
  ...
  if (blk == BlkNum_invalid) {
    if (!this->WriteFlash()) return SegBlkNum_invalid;
...
bool BlkWriter::WriteFlash(void) {
  ...
  if (!need_write) {
    Logging::Log(kLLInfo, kLCLog, "BlkWriter::WriteFlash(cur_seg=%"PRIu32") !need_write", cur_seg);
    return true;
  }
  
  SegNum next_seg = this->allocator()->Next();
  if (next_seg == SegNum_invalid) {
    Logging::Log(kLLWarn, kLCLog, "BlkWriter::WriteFlash(cur_seg=%"PRIu32",next_seg=invalid)", cur_seg);
    return false;
  }
  Logging::Log(kLLInfo, kLCLog, "BlkWriter::WriteFlash(cur_seg=%"PRIu32",next_seg=%"PRIu32")", cur_seg, next_seg);
