...
Superblock* Log::create_superblock(void) {
  ...
  if (v->Read(this->flash())) return v;
  delete v;
  return NULL;
}

SegIO* Log::create_segio(void) {
  return new SegIO(this->flash(), this->superblock(), this->segcache_capacity());
}
...
bool Log::WriteCheckpointCond(CheckpointReason reason) {
  ...
  if (reason != kCheckpointReason_NewSeg) {
    this->bw()->WriteFlash();//triggers upcall, which in turn calls WriteCheckpointCond(NewSeg)
    return false;
  }
