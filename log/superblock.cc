...
void Superblock::set_nsector_blk(FlashSectorNum value) { this->nsector_blk_ = value; }
...
void Superblock::set_nseg_flash(SegNum value) { this->nseg_flash_ = value; }
...
bool Superblock::Read(Flash* flash) {
  FlashSectorNum nsector_blk = this->FindBlockSize(flash);
  if (nsector_blk == 0) return false;
  FlashSectorNum seg0blk2 = 2 * nsector_blk;
  size_t noctet_blk = nsector_blk * flash->noctet_sector();
  uint8_t* buf = (uint8_t*)::malloc(noctet_blk);
  bool success = flash->Seek(seg0blk2) && flash->Read(nsector_blk, buf)
                 && this->Read(buf, noctet_blk);
  ::free(buf);
  return success;
}

FlashSectorNum Superblock::FindBlockSize(Flash* flash) {
  uint8_t* buf = (uint8_t*)::malloc(flash->noctet_sector());
  FlashSectorNum i = 1; FlashSectorNum nsector_blk = 0;
  for (; i <= 1025; ++i) {
    flash->Seek(i);
    flash->Read(1, buf);
...
bool Superblock::Read(uint8_t* buf, size_t noctet_blk) {
  ...
  if (!(
    ...
    p.has_nblk_seg() && p.has_nseg_flash() &&
  ...
  this->set_nseg_flash(p.nseg_flash());
...
bool Superblock::Write(uint8_t* buf) const {
  ...
  p.set_nseg_flash(this->nseg_flash());
