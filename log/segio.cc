...
SegIO::SegIO(Flash* flash, Superblock* superblock, SegNum cache_capacity) {
  assert(flash != NULL);
...
bool SegIO::Write(SegNum seg, const uint8_t* buf) {
  ...
  return this->flash()->Write(this->superblock()->nsector_seg(), buf);
}
...
bool SegIO::SeekTo(SegNum seg) {
  FlashSectorNum sector = seg * this->superblock()->nsector_seg();
  return this->flash()->Seek(sector);
}
...
charbuf* SegIO::ReadSeg(SegNum seg) {
  ...
  if (c == NULL) {
    ...
    if (!this->flash()->Read(this->superblock()->nsector_seg(), buf)) {
      free(buf);
      return NULL;
    }
