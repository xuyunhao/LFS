...
Segment::Segment(Flash flash, int seg_id, int start_sector_id, int pre_seg_size, int blk_per_seg, int sector_per_blk, int wearlimit, Segment * prev) {
    ...
    for (int i = 0; i < blk_per_seg- this->pre_seg_size; i++) {
        int id = i + this->pre_seg_size;
        int start = start_sector_id + id * sector_per_blk;
        Block * blk = new Block(this->flash, start, sector_per_blk, id, wearlimit, this->current_usage);
        (this->blk_list).push_back(blk);
        delete blk;
    }
}
...
bool Segment::set_new_blk(int inode_id, int inode_v, char * s) {
    if (this->next_free_blk_id > this->maximum_size) {
        ...
        bool r = ((this->blk_list)[id])->set_content(s, inode_id, inode_v);
        delete blk;
        return r;
    }
...
bool Segment::write_to_flash() {
    ...
    std::cout << "Segment::write_to_flash()" << std::endl;
    print_seg(seg_metadata);
    bool r = Flash_Write(this->flash, this->sector_offset, this->pre_seg_size, seg_metadata);
    delete seg_metadata;
    return r;
}
